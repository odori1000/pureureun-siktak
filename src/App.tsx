...
import { TOSS_PRODUCTS, type TossProduct } from './lib/toss';
...
const VEGETABLE_PRODUCTS: ProductDetail[] = [
  {
      ...
      <section id="products-section" className="relative min-h-screen bg-[#A5C3CF] py-32 px-6 flex flex-col justify-center">
        ...
        <div className="max-w-6xl mx-auto w-full relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-10">
            {VEGETABLE_PRODUCTS.map((prod) => (
              <ProductCard key={prod.tossId} detail={prod} />
            ))}
          </div>
        </div>
      </section>
...
function ProductCard({ detail }: ProductCardProps) {
  ...
  // 기본 TossProduct 정보 찾기
  const baseTossProduct = TOSS_PRODUCTS.find((p) => p.id === detail.tossId) || {
    id: detail.tossId,
    name: detail.name,
    price: 3000,
    currency: 'KRW',
  ...
  const finalPrice = baseTossProduct.price + currentOption.priceOffset + (currentSecondaryOption ? currentSecondaryOption.priceOffset : 0);
    ...
    >
      ...
      <div className="mt-auto">
        ...
        <TossCheckoutButton
          ...
          className="bg-emerald-600 hover:bg-emerald-500 active:scale-[0.98] transition-all duration-200"
